...
7. ВАЖНО: НЕ менять vite.config.ts base — он должен быть `'./'`, не `/crew_editor/`

## Известные проблемы
- vite.config.ts: base должен быть `./`. CrewAI агенты любят перезаписывать на `/crew_editor/` — deploy.sh чинит это автоматически
